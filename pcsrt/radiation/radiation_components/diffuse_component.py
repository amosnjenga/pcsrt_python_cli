...
def get_diffuse_irradiance(solar_altitude,incline_angle,normal_vector: np.array,solar_distance_variation_correction,
    linke_turbidity_factor,beam_component):
    if len(linke_turbidity_factor) == 1:
        linke_turbidity_factor = linke_turbidity_factor[0]

    diffuse_transmission_function = -0.015843 \
        + 0.030543 * linke_turbidity_factor \
        + 0.0003797 * linke_turbidity_factor ** 2.

    a_0 = 0.2646 - 0.061581 * linke_turbidity_factor \
        + 0.0031408 * linke_turbidity_factor ** 2.
    if a_0 < 0.002:
        a_0 = 0.002 / diffuse_transmission_function

    a_1 = 2.0402 + 0.018945 * linke_turbidity_factor \
        - 0.011161 * linke_turbidity_factor ** 2.

    a_2 = -1.3025 \
        + 0.039231 * linke_turbidity_factor \
        + 0.0085079 * linke_turbidity_factor ** 2.

    diffuse_angular_function = a_0 \
        + a_1 * np.sin(solar_altitude) \
        + a_2 * np.sin(solar_altitude) ** 2.

    diffuse_irradiance = SOLAR_CONSTANT \
        * solar_distance_variation_correction \
        * diffuse_transmission_function \
        * diffuse_angular_function
    slope = np.arccos(np.dot(normal_vector.as_numpy_vec(), np.array([normal_vector.x, normal_vector.y, 1.])))
